...
def X_reg_inverse_M_adaptive_damping(U,D,M,lambdda):
...
def M_X_reg_inverse_adaptive_damping(U,D,M,lambdda):
    # X = UDU^T; want to compute (X + lambda I)^{-1}M
    # X is low rank! X is square: X is either AA^T or GG^T
...
class RS_KFACOptimizer(optim.Optimizer):
    def __init__(self,
                 model,
        ...
        super(RS_KFACOptimizer, self).__init__(model.parameters(), defaults)
    ...
    def _get_natural_grad(self, m, p_grad_mat, damping):
        ...
        v1 = X_reg_inverse_M_adaptive_damping(U = self.Q_g[m], D = self.d_g[m], M = p_grad_mat, lambdda = damping) # the damping here is adaptive!
        v = M_X_reg_inverse_adaptive_damping(U = self.Q_a[m], D = self.d_a[m], M = v1, lambdda = damping)  # the damping here is adaptive!
